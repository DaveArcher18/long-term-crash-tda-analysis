...
\begin{abstract}
This study evaluates the Gidea-Katz (2017) Topological Data Analysis methodology for predicting stock market crashes over 69 years of US market history (1957--2025). Using persistence landscape norms computed from 3D point clouds of daily log-returns (DJI, DJT, S\&P 500) and Mann-Kendall trend detection, we find statistically significant predictive power: 20\% strict precision versus 13\% base rate, with 38\% recall. The technique excels at detecting slow-building systemic crises (1973--74: 40\%, 1987: 100\%, 2008: 49\%, 2022: 50\% warning rates) but fails on exogenous shocks (1962: 0\%, COVID-2020: 0\%). We conclude that TDA is a valid supplementary risk indicator but should not be used as a standalone trading signal.
\end{abstract}
...
This study evaluates the Gidea-Katz (2017) Topological Data Analysis (TDA) methodology for predicting stock market crashes over 69 years of US market history (1957--2025). The technique computes persistence landscape norms from 3D point clouds of daily log-returns and uses Mann-Kendall trend detection to identify periods of rising topological instability.

\subsection*{Key Findings}

\begin{itemize}[noitemsep]
    \item The TDA signal demonstrates \textbf{statistically significant predictive power} with 20\% strict precision (vs.\ 13\% base rate) and 38\% recall
    \item When including corrections (10--20\% drawdowns), precision rises to \textbf{44.6\%}---nearly half of all warnings precede meaningful market declines
    \item The technique excels at detecting \textbf{slow-building systemic crises} (1973--74: 40\%, 1987: 100\%, 2008: 49\%, 2022: 50\% warning rates)
    \item It fails on \textbf{exogenous shocks} (1962: 0\%, COVID-2020: 0\% warning rates)
\end{itemize}

\subsection*{Recommendation}

The TDA methodology is a valid supplementary risk indicator for detecting building financial instability, but should not be used as a standalone trading signal due to high false positive rates (14\% of all days).
...
Can topological features extracted from multi-index return dynamics provide early warning signals for major market crashes?

\subsection{Background}

Gidea and Katz (2017) proposed using Topological Data Analysis to detect market instability by measuring the complexity of return patterns across multiple indices. Their hypothesis: before major crashes, the ``shape'' of market dynamics becomes increasingly complex, manifesting as elevated persistence in $H_1$ homology (1-dimensional loops in the return point cloud).

The persistence landscape, introduced by Bubenik (2015), provides a stable, functional representation of persistence diagrams that admits statistical analysis. The $L^1$ and $L^2$ norms of these landscapes serve as scalar measures of topological complexity.
...
\begin{itemize}[noitemsep]
    \item \textbf{7 major market crashes} (1962, 1973--74, 1987, 2000, 2008, 2020, 2022)
...
Data was obtained from Stooq.com, which provides historical index data back to the 1950s.
...
\begin{enumerate}[noitemsep]
    \item \textbf{Look-ahead bias in classification}: Forward drawdowns are computed with future data (unavoidable for evaluation)
    ...
    \item \textbf{No transaction costs}: Metrics don't account for implementation frictions
\end{enumerate}
...
Warning days & 4,344 (25.4\%) & Signal triggered $\sim$1 in 4 days \\
...
\midrule
1962 Flash Crash & May--Jun 1962 & 0\% & 21.4\% & \textcolor{failred}{Missed} \\
1973--74 Bear Market & 1973--1974 & 40.2\% & 44.1\% & \textcolor{successgreen}{Detected} \\
1987 Black Monday & Aug--Oct 1987 & 100\% & 33.5\% & \textcolor{successgreen}{Strong} \\
2000 Dot-com Bust & Mar--Apr 2000 & 7.1\% & 27.2\% & \textcolor{warningorange}{Weak} \\
2008 Financial Crisis & Sep--Nov 2008 & 49.2\% & 47.0\% & \textcolor{successgreen}{Detected} \\
2020 COVID Crash & Feb--Mar 2020 & 0\% & 33.9\% & \textcolor{failred}{Missed} \\
2022 Bear Market & Jan--Oct 2022 & 50.2\% & 25.4\% & \textcolor{successgreen}{Detected} \\
\bottomrule
\end{tabular}
...
Figure~\ref{fig:dashboard} presents the comprehensive backtest dashboard showing persistence landscape norms, Mann-Kendall $\tau$ values, forward drawdowns, and classification summary across the full 69-year period.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures/century_dashboard.png}
\caption{TDA Crash Prediction Backtest Dashboard (1957--2025)}
\label{fig:dashboard}
\end{figure}
...
The TDA methodology detects \textbf{rising complexity in inter-index return dynamics}. When the three indices (industrial, transportation, broad market) begin moving in non-linear, asynchronous patterns, the persistence landscape norms increase.

Examining the time series of $L^1$ norms reveals:
...
\begin{description}[noitemsep]
    \item[1973--74 Bear Market:] 18-month deterioration from oil shock, inflation, Nixon resignation. Gradual stress buildup visible in topological metrics.
    \item[1987 Black Monday:] August--October buildup with rising volatility and inter-index divergence. 100\% warning rate indicates clear topological signal.
    \item[2008 Financial Crisis:] Subprime problems emerged in 2007, giving $>$12 months of building stress. Topological instability tracked credit deterioration.
...
\begin{enumerate}[noitemsep]
    \item \textbf{Near-misses}: Some ``false positives'' occurred during genuine stress that was resolved (e.g., policy intervention)
    \item \textbf{Regime sensitivity}: Higher false positive rates during structurally volatile periods (1970s, early 2000s)
    \item \textbf{Threshold dependency}: The $\tau > 0.30$ threshold may be suboptimal; higher thresholds reduce false positives but miss more true events
\end{enumerate}
...
\midrule
Scientific validity & \textcolor{successgreen}{\checkmark} Demonstrated statistically significant predictive power \\
...
\begin{enumerate}[noitemsep]
    ...
    \item \textbf{Multi-decade robustness}: Works across different market regimes (1957--2025)
...
\begin{enumerate}[noitemsep]
    ...
    \item \textbf{Blind to exogenous shocks}: Cannot predict pandemics, geopolitical events, technical failures
...
\begin{enumerate}[noitemsep]
    \item \textbf{Use as supplementary indicator}: Combine with fundamental, sentiment, and technical signals---not standalone
    ...
    \item \textbf{Do not rely for tail risk}: Cannot protect against sudden exogenous events
\end{enumerate}
...
\begin{enumerate}[noitemsep]
    \item Gidea, M., \& Katz, Y. (2017). Topological Data Analysis of Financial Time Series: Landscapes of Crashes. \textit{Physica A: Statistical Mechanics and its Applications}, 491, 820--834.
...
Pipeline runtime: $\sim$1.4 minutes for 17,318 observations on 7 cores.
